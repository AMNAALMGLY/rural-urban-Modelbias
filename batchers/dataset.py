...
class Batcher(torch.utils.data.IterableDataset):
    ...
    def __init__(self, tfrecords, scalar_features_keys, ls_bands, nl_bands, label, nl_label, normalize='DHS',
        ...
        self.ds = get_dataset(tfrecords, batch_size, label,nl_label,ls_bands,nl_bands,scalar_features_keys,clipn,normalize,cache, shuffle, groupby, augment,
                )
...
def tfrecords_to_dict(label,nl_label,ls_bands,nl_bands,scalar_features_keys,clipn,normalize, example: tf.Tensor) -> dict[str, tf.Tensor]:
        '''
        convert the records to dictionaries of {'feature_name' : tf.Tensor}
...
def get_dataset(tfrecords, batch_size, label,nl_label,ls_bands,nl_bands,scalar_features_keys,clipn,normalize, cache, shuffle, groupby, augment,
    ...
    dataset = dataset.map(lambda ex:  tfrecords_to_dict(label,nl_label,ls_bands,nl_bands,scalar_features_keys,clipn,normalize, ex), num_parallel_calls=args.num_workers)
    if groupby == 'urban':
    ...
    if augment:
        ...
        dataset = tf.data.Dataset.zip((dataset, (counter, counter)))
        seed=123
        dataset = dataset.map(lambda ex :augment_ex( ex,seed,nl_bands,ls_bands,nl_label) , num_parallel_calls=args.num_workers)
